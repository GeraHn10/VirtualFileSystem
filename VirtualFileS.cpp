	...
	if (!archivo) {
		...
		int totalE = inf.totalEntradas;
			...
			for (int i = 0; i < totalE ; i++) {
				...
				if (i==padre && (as.FileType == 'd'|| as.FileType ==  'a') ){
...
void vfs::diskInfo3(int padre) {
	...
	if (!archivo) {
		...
		int totalE = inf.totalEntradas;
		FileEntry PS;
		archivo.seekg(size +(sizeof(FileEntry)*padre));
		archivo.read(reinterpret_cast<char *>(&as),sizeof(FileEntry));
		archivo.seekp(-(sizeof(FileEntry)-archivo.tellp()));
		archivo.write(reinterpret_cast<const char *>(&PS),sizeof(FileEntry));
...
bool vfs::esarchivo(int padre) {
	FileEntry as;
	fstream archivo("VirtualFile.bin", ios::out | ios::in | ios::binary);
	archivo.seekg(sizeof(MetaData) + sizeof(bitMap) + (sizeof(FileEntry)*padre));
	archivo.read(reinterpret_cast<char *>(&as), sizeof(FileEntry));

	return as.FileType == 'a';
		...
		for (int i = 0; i < posi+1; i++) {
			...
			if (p == 12) {
				asE.seekg(sizeof(MetaData) + sizeof(bitMap) + (sizeof(FileEntry)*totalE) + ((sizeof(DataBlock_Direct) * 12)
					+ sizeof(DataBlock_I1) + sizeof(DataBlock_I2) + sizeof(DataBlock_I3)*padre) + (sizeof(DataBlock_Direct)*12));
				...
				while (!arL.eof() && j < 4096) {
					...
					j = j + 1;
					if (j == 4096) {
						asE.seekp(-(sizeof(DataBlock_I1) - asE.tellp()));
						asE.write(reinterpret_cast<const char *> (&ing), sizeof(DataBlock_I1));
						Bp1 = Bp1 + 1;
						j = 0;
					}
					if (Bp1 == 15) {
				...
				asE.seekg(sizeof(MetaData) + sizeof(bitMap) + (sizeof(FileEntry)*totalE) + ((sizeof(DataBlock_Direct) * 12)
					+ sizeof(DataBlock_I1) + sizeof(DataBlock_I2) + sizeof(DataBlock_I3)*padre)+(sizeof(DataBlock_Direct) * 12) + sizeof(DataBlock_I1) );
				asE.read(reinterpret_cast<char *>(&ing2), sizeof(DataBlock_I2));

				while (!arL.eof() && j < 4096) {

					arL.get(ing2.points[Bp2].data[j]);
					bitmap[(15 * padre) + 13] = '1';
					j = j + 1;
					if (j == 4096) {
						asE.seekp(-(sizeof(DataBlock_I2) - asE.tellp()));
						asE.write(reinterpret_cast<const char *> (&ing2), sizeof(DataBlock_I2));
						Bp2 = Bp2 + 1;
						j = 0;
					}
					if (Bp2 == 32) {
					...
					if(arL.eof()){
						...
						asE.seekg(sizeof(MetaData) + sizeof(bitMap) + (sizeof(FileEntry)*totalE) + ((sizeof(DataBlock_Direct) * 12)
							+ sizeof(DataBlock_I1) + sizeof(DataBlock_I2) + sizeof(DataBlock_I3)*padre) + (sizeof(DataBlock_Direct) * 12) + sizeof(DataBlock_I1) +sizeof(DataBlock_I2));
						asE.read(reinterpret_cast<char *>(&ing3), sizeof(DataBlock_I3));

						while (!arL.eof() && j < 4096) {

							arL.get(ing2.points[Bp3].data[j]);
							bitmap[(15 * padre) + 14] = '1';
							j = j + 1;
							if (j == 4096) {
								asE.seekp(-(sizeof(DataBlock_I3)-asE.tellp()));
								asE.write(reinterpret_cast<const char *> (&ing3),sizeof(DataBlock_I3));
								Bp3 = Bp3 + 1;
								j = 0;
							}
							if (Bp3 == 64) {
				...
				asE.seekg(sizeof(MetaData) + sizeof(bitMap) + (sizeof(FileEntry)*totalE) + (sizeof(DataBlock_Direct) * p));
				asE.read(reinterpret_cast<char *>(&infor), sizeof(DataBlock_I1));
				while (!arL.eof() && j < 4096) {

					arL.get(infor.data[j]);
					bitmap[(15 * padre) + p] = '1';
					j = j + 1;
					if (j == 4096) {

						asE.seekp(-(sizeof(DataBlock_I2) - asE.tellp()));
						asE.write(reinterpret_cast<const char *> (&ing), sizeof(DataBlock_Direct));
						j = 0;
						p = p + 1;
					}
					if (arL.eof()) {
...
void vfs::exportar(int padre) {
	...
	if (!archivo) {
		...
		int size=sizeof(MetaData) + sizeof(bitMap) + (sizeof(FileEntry)*totalE) + ((sizeof(DataBlock_Direct) * 12)
			+ sizeof(DataBlock_I1) + sizeof(DataBlock_I2) + sizeof(DataBlock_I3)*padre);
		
		for (int j = 0; j < 15; j++) {
			cout << posL[j] << endl;
			if (posL[j]==12) {
			
				archivo.seekg(size+(sizeof(DataBlock_Direct)*12));
				archivo.read(reinterpret_cast<char *>(&ing), sizeof(DataBlock_I1));
				cout << ing.points << endl;
			}
			if (posL[j] == 13) {

				archivo.seekg(size + (sizeof(DataBlock_Direct) * 12)+sizeof(DataBlock_I1));
				archivo.read(reinterpret_cast<char *>(&ing2), sizeof(DataBlock_I2));
				cout << ing2.points << endl;
			}
			if (posL[j] == 13) {

				archivo.seekg(size + (sizeof(DataBlock_Direct) * 12) + sizeof(DataBlock_I1) + sizeof(DataBlock_I2));
				archivo.read(reinterpret_cast<char *>(&ing3), sizeof(DataBlock_I3));
				cout << ing3.points << endl;
			}
			else {
				archivo.seekg(size + (sizeof(DataBlock_Direct) * posL[j]) );
				archivo.read(reinterpret_cast<char *>(&infor), sizeof(DataBlock_Direct));
				cout << infor.data << endl;
